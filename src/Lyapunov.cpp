...
void Lyapunov::generateSequence(){
    ...
    for(unsigned int i = 0;i< m_sequence.length();i++){
      std::cout << m_sequence.at(i) << " ici "<< std::endl;
      if (not(m_sequence.at(i) == 'A' || m_sequence.at(i) == 'B')){
        std::cout <<m_sequence << m_sequence.length()<< "An error in the construction of the sequence has been detected. Sequence must contains only A and B. Default Sequence : AB" << std::endl;
        error = true;
        break;
      }
    }
