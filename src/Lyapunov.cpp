...
void Lyapunov::setPixels(std::vector<Uint32> pixels){
    windowManager.setPixels(pixels.data());
    windowManager.update();
}

Lyapunov::Lyapunov() : windowManager(HEIGHT, WIDTH), pixels(){
...
std::vector<Uint32> exposant(std::string seq){
...
int main(){
    ...
    seq = AB(seq);
    Lyapunov lyapunov;
    std::vector<Uint32> arrayExpo = exposant(seq);
    std::cout << "Generation finie\n";
    lyapunov.setPixels(arrayExpo);
    lyapunov.eventLoop();
    return EXIT_SUCCESS;
}
